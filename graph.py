...
def create_weighted_graph(y_true: Tensor, graph: nx.Graph, source: int, target: int):
    ...
    node_labels = {node: prediction for node, prediction in zip(graph.nodes, predictions)}
    
    edge_weights = {(u, v): 1 / prediction for (u, v), prediction in zip(graph.edges, predictions)}
    ...
    edge_colors = ["red" if edge in zip(shortest_path, shortest_path[1:]) else "black" for edge in graph.edges]
